...
from audio import spec as spectrogram
from data import data_utils
from omegaconf import OmegaConf


def compute_spec(hps, filename, specname):
    audio, sampling_rate = data_utils.load_wav_to_torch(filename)
...
def process_file_spec(hps, wavPath, outpath):
    if wavPath.endswith(".wav"):
        file = wavPath.split('/')[-1].split('.')[0]
        compute_spec(hps.data, f"{wavPath}", f"{outpath}/{file}.pt")
